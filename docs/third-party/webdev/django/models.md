...
3. Los campos del modelo son _atributos de clase_.
4. Es muy ^^recomendable^^ definir el método `#!python __str__()`
...
=== ":one: Crear migración"
    ...
    === "*venv* :octicons-package-24:{.blue}"
        ...
        :material-check-all:{ .blue } Es posible crear las migraciones de una aplicación concreta utilizando el comando: `./manage.py makemigrations app`
    ...
    === "*uv* &nbsp;:simple-uv:{.uv}"
        ...
        :material-check-all:{ .blue } Es posible crear las migraciones de una aplicación concreta utilizando el comando: `uv run manage.py makemigrations app`
...
=== ":two: Aplicar migración"
    ...
    === "*venv* :octicons-package-24:{.blue}"
        ...
        :material-check-all:{ .blue } Es posible aplicar las migraciones de una aplicación concreta utilizando el comando: `./manage.py migrate app`
    ...
    === "*uv* &nbsp;:simple-uv:{.uv}"
        ...
        :material-check-all:{ .blue } Es posible aplicar las migraciones de una aplicación concreta utilizando el comando: `uv run manage.py migrate app`
...
Para crear un nuevo objeto (y almacenarlo en la base de datos) mediante el ORM de Django, podemos usar dos métodos:
...
=== "`save()`"
    ...
    ...     content='Awesome features in the last release of Django'
    ... )
    >>> p.save()#(3)!
    ```
    { .annotate }
...
1|Check out the new Django version|Awesome features in the last release of Django
```
...
#### Construyendo consultas { #build-queries }
...
A continuación se muestran algunos de los selectores más destacados:
...
| [`contains`](https://docs.djangoproject.com/en/stable/ref/models/querysets/#contains) | Busca si contiene el término. |
...
| [`in`](https://docs.djangoproject.com/en/stable/ref/models/querysets/#in) | Busca si aparece en conjunto de valores. |
...
| [`startswith`](https://docs.djangoproject.com/en/stable/ref/models/querysets/#startswith) | Busca si empieza por un término. |
