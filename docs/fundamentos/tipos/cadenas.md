...
![Banner](images/cadenas/banner.jpg)
/// caption
...
1. Otra opción es [escapar](#secuencias-de-escape) las comillas simples: `#!python 'Los llamados \'strings\' son secuencias de caracteres'`.
...
Para el caso contrario de convertir un «string» a un valor numérico, tenemos a disposición las [funciones ya vistas](numeros.md#conversion-explicita):
...
:material-check-all:{ .blue } Los <span class="example">ejemplos:material-flash:</span> anteriores se han mostrado con el [intérprete de Python](datos.md#conocer-el-valor-de-una-variable). Aún así hemos utilizado la función `#!python print()` porque nos permite ver realmente el resultado de utilizar los caracteres escapados.
...
!!! note "Expresiones regulares"
    ...
    El modificador `#!python r''` es muy utilizado para escribir [expresiones regulares](../../libreria/texto/re.md).
...
1. La función `#!python input()` siempre nos **devuelve un objeto de tipo cadena de texto** o `str`. Tenerlo muy en cuenta a la hora de trabajar con números, ya que debemos realizar una [conversión explícita](numeros.md#conversion-explicita).

!!! warning "Advertencia"

    Aunque está permitido, **NUNCA** llames `input` a una variable porque destruirías la función que nos permite leer datos desde teclado. Y tampoco uses nombres derivados como `_input` o `input_` ya que **no son nombres representativos** que [identifiquen el propósito de la variable](datos.md#convenciones-para-nombres).

!!! exercise "Ejercicio"

    [pypas](../../paquetes/aprendizaje/pypas.md) &nbsp;:fontawesome-solid-hand-holding-heart:{ .acc .slide } `calc-basic`    
...
![Dark image](images/cadenas/string-indexing-dark.svg#only-dark)
![Light image](images/cadenas/string-indexing-light.svg#only-light)
...
Las cadenas de texto son tipos de datos [inmutables](datos.md#mutabilidad). Es por ello que no podemos modificar un carácter directamente.
...
!!! failure "Sobre las constantes"
    ...
    No hay que confundir las [constantes](datos.md#constantes) con los tipos de datos inmutables. Es por ello que las variables que almacenan cadenas de texto, a pesar de ser inmutables, no se escriben en mayúsculas.
...
=== "Comienzo y fin"
    ...
    ![Dark image](images/cadenas/string-slicing-dark.svg#only-dark)
    ![Light image](images/cadenas/string-slicing-light.svg#only-light)
...
!!! exercise "Ejercicio"
    ...
    [pypas](../../paquetes/aprendizaje/pypas.md) &nbsp;:fontawesome-solid-hand-holding-heart:{ .acc .slide } `half-turn`
...
2. Esta es la forma realmente [pitónica](../modularidad/funciones.md#hazlo-pitonico).
...
!!! tip "Cadena modificada"
    ...
    La función `strip()` no modifica la cadena que estamos usando (_algo obvio porque los «strings» son [inmutables](datos.md#mutabilidad)_) sino que ^^devuelve una nueva cadena^^ de texto con las modificaciones pertinentes.

### Realizar búsquedas

Aunque hemos visto que la forma [pitónica](../modularidad/funciones.md#hazlo-pitonico) de saber [si una subcadena se encuentra dentro de otra](#pertenencia-de-un-elemento) es a través del operador `#!python in`, Python nos ofrece distintas alternativas para realizar búsquedas dentro de una cadena de texto.
...
!!! exercise "Ejercicio"
    ...
    [pypas](../../paquetes/aprendizaje/pypas.md) &nbsp;:fontawesome-solid-hand-holding-heart:{ .acc .slide } `lost-word`
...
=== "Formateando en otras bases"
    ...
    Como ^^curiosidad^^, si utilizamos el modificador `X` **en mayúsculas** es como si aplicáramos (automáticamente) un [`upper()`](#mayusculas-y-minusculas) al resultado del valor _hexadecimal_:
    ...
    !!! note "Cambio de base"
        ...
        Nótese la diferencia de obtener el cambio de base con este método frente a las [funciones de cambio de base](numeros.md#bases) ya vistas previamente que añaden el prefijo de cada base `#!python 0b`, `#!python 0o` y `#!python 0x`.
...
!!! exercise "Ejercicio"
    ...
    [pypas](../../paquetes/aprendizaje/pypas.md) &nbsp;:fontawesome-solid-hand-holding-heart:{ .acc .slide } `fstring-format`
...
1. Un **código numérico** único (habitualmente se muestra en [hexadecimal](numeros.md#base-hexadecimal)).
...
!!! exercise "Ejercicio"
    ...
    [pypas](../../paquetes/aprendizaje/pypas.md) &nbsp;:fontawesome-solid-hand-holding-heart:{ .acc .slide } `find-unicode`
...
1. [pypas](../../paquetes/aprendizaje/pypas.md) &nbsp;:fontawesome-solid-hand-holding-heart:{ .acc .slide } `format-hexcolor`
2. [pypas](../../paquetes/aprendizaje/pypas.md) &nbsp;:fontawesome-solid-hand-holding-heart:{ .acc .slide } `swap-name`
3. [pypas](../../paquetes/aprendizaje/pypas.md) &nbsp;:fontawesome-solid-hand-holding-heart:{ .acc .slide } `samba-split`
4. [pypas](../../paquetes/aprendizaje/pypas.md) &nbsp;:fontawesome-solid-hand-holding-heart:{ .acc .slide } `nif-digit`
5. [pypas](../../paquetes/aprendizaje/pypas.md) &nbsp;:fontawesome-solid-hand-holding-heart:{ .acc .slide } `n-repeat`
6. [pypas](../../paquetes/aprendizaje/pypas.md) &nbsp;:fontawesome-solid-hand-holding-heart:{ .acc .slide } `str-metric`
7. [pypas](../../paquetes/aprendizaje/pypas.md) &nbsp;:fontawesome-solid-hand-holding-heart:{ .acc .slide } `h2md`
8. [pypas](../../paquetes/aprendizaje/pypas.md) &nbsp;:fontawesome-solid-hand-holding-heart:{ .acc .slide } `count-sheeps`
9. [pypas](../../paquetes/aprendizaje/pypas.md) &nbsp;:fontawesome-solid-hand-holding-heart:{ .acc .slide } `strip1`
10. [pypas](../../paquetes/aprendizaje/pypas.md) &nbsp;:fontawesome-solid-hand-holding-heart:{ .acc .slide } `find-integral`
11. [pypas](../../paquetes/aprendizaje/pypas.md) &nbsp;:fontawesome-solid-hand-holding-heart:{ .acc .slide } `multiply-jack`
12. [pypas](../../paquetes/aprendizaje/pypas.md) &nbsp;:fontawesome-solid-hand-holding-heart:{ .acc .slide } `first-last-digit`
