...
SQLite no dispone de un tipo especial para valores [booleanos](../../fundamentos/tipos/numeros.md#booleanos). Usaremos el tipo `INTEGER` para representar _true_ como **1** y _false_ como **0**.

#### Fecha y hora

SQLite no dispone de un tipo especial para _fecha y hora_. Usaremos el tipo `TEXT` para representar fecha y hora en formato [`ISO8601`](https://iso8601.com/) como [cadenas de texto](../../fundamentos/tipos/cadenas.md) `#!python 'YYYY-MM-DD HH:MM:SS.SSS'`.
...
1.  - Es habitual usar [comillas triples](../../fundamentos/tipos/cadenas.md#comillas-triples) para definir sentencias SQL dentro de código Python.
    - No es necesario añadir punto y coma `;` al final de la sentencia SQL cuando usamos el módulo `sqlite3` salvo que se trate de [scripts](#ejecucion-de-scripts).
...
En una primera aproximación podríamos pensar en utilizar un [f-string](../../fundamentos/tipos/cadenas.md#f-strings).
...
La primera aproximación que se nos podría venir a la cabeza es utilizar una [inserción parametrizada](#insercion-parametrizada) por cada línea del fichero de entrada:
...
=== "Iterable de listas"
    ...
    2.  - Hemos conseguido generar una **lista de listas**.
        - Es posible que no veas la salida exactamente así. Si es el caso, puedes hacer lo siguiente:
            ...
            >>> pprint(pyversions)
            ```
    3. Utilizamos una [inserción parametrizada](#insercion-parametrizada) usando «placeholders» posicionales.
...
=== "Iterable de diccionarios"
    ...
    2.  - Hemos conseguido generar una **lista de diccionarios**.
        - Es posible que no veas la salida exactamente así. Si es el caso, puedes hacer lo siguiente:
            ...
            >>> pprint(pyversions)
            ```
    3. Utilizamos una [inserción parametrizada](#insercion-parametrizada) usando «placeholders» nominales.
...
En SQLite también es posible utilizar un [gestor de contexto](../../fundamentos/modularidad/poo.md#gestores-de-contexto) sobre la conexión, que funciona de la siguiente manera:
...
Se ha elevado una [excepción](../../fundamentos/modularidad/excepciones.md) de tipo `IntegrityError` indicando que hay valores duplicados en el campo `branch` ya que se trata de una clave primaria y sus valores deben ser únicos. Pero dado que estamos en un gestor de contexto, se realiza un «rollback» de las acciones previas y la base de datos queda en el mismo estado anterior. Es decir, no se inserta ninguna fila nueva.
...
La ejecución de una consulta —desde un cursor— retorna un [objeto iterable](../../fundamentos/modularidad/poo.md#iterables). Por defecto el contenido de dicho iterable son **tuplas** donde cada _tupla_ representa una fila de la tabla consultada.
...
1. Nótese que la llamada a `fetchone()` hace que quede «una fila menos» que recorrer. Es un comportamiento totalmente análogo al de la [lectura de una línea](../../fundamentos/estructuras/ficheros.md#lectura-de-una-linea) en un fichero.
...
La aplicación del [operador morsa](../../fundamentos/flujo/condicionales.md#operador-morsa) en las consultas mediante `sqlite3` es muy habitual ya que facilita la comprobación de resultados.
...
1. Se utiliza un [gestor de contexto](#gestor-de-contexto).
...
1. Es posible aplicar [gestores de contexto](#gestor-de-contexto) sobre dos conexiones a la vez.
...
1. [pypas](../../paquetes/aprendizaje/pypas.md) &nbsp;:fontawesome-solid-hand-holding-heart:{ .acc .slide } `todo`
2. [pypas](../../paquetes/aprendizaje/pypas.md) &nbsp;:fontawesome-solid-hand-holding-heart:{ .acc .slide } `twitter`
