...
Como se ha [comentado previamente](arranque.md#interfaz-administrativa) Django proporciona una interfaz administrativa para manejar los modelos de nuestro proyecto, prácticamente sin esfuerzo.
...
Cuando [registramos un modelo](#registrar-un-modelo), sus objetos aparecen en la interfaz administrativa a través del método `__str__()`. Si no está implementado veremos algo como: `#!python <Post: Post object (1)>`
...
Como <span class="example">ejemplo:material-flash:</span> vamos a partir de [este escenario](modelos.md#relaciones-uno-a-muchos) en el que un «post» puede tener varios comentarios.
...
Pero podemos mejorarlo muy fácilmente. Partiendo del <span class="example">ejemplo:material-flash:</span> en el que [un «post» puede tener varias etiquetas](modelos.md#relaciones-muchos-a-muchos), haríamos lo siguiente:
...
Para ilustrar el modo de uso con un <span class="example">ejemplo:material-flash:</span>, vamos a retomar [este escenario](modelos.md#relaciones-muchos-a-muchos-con-modelo-intermedio) en el que un «post» puede tener varias etiquetas y se añade el **detalle** del etiquetado (al asignar etiquetas a «posts»):
...
Obviamente también podemos hacer uso de `raw_id_fields` o `autocomplete_fields` ([relaciones 1:N](#relaciones-uno-a-muchos)) en la clase `PostLabelingDetailInline` con el objetivo de mejorar la búsqueda de etiquetas.
