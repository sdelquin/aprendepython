...
Hay una serie de aplicaciones que **ya vienen incluidas en la propia instalación de Django**. En el [primer arranque](arranque.md#primer-arranque) vimos que se aplicaron [migraciones](modelos.md#migraciones) de:
...
=== "*uv* &nbsp;:simple-uv:{.uv}"
    ...
    ??? abstract "justfile"
        ...
        Consulta la receta [`startapp`](justfile.md#justfile-para-django) para incluirla en tu `justfile`.
...
En este sentido **es recomendable** [crear una aplicación](#crear-una-aplicacion) `shared`{ .red } que contenga artefactos compartidos: plantillas base, estáticos comunes, funciones genéricas, etc.
