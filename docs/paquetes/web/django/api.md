...
- Se utilizan **métodos HTTP** para indicar la [operación a realizar](#diseno).
...
[Django Ninja](https://django-ninja.dev/) es un _framework_ para construir APIs con [Django](../django/) y [anotaciones de tipo](../../../fundamentos/modularidad/funciones.md#anotacion-de-tipos) en Python.
...
- [x] **Interconexión con Django**: (Obviamente) tiene una buena integración con Django y su [ORM](modelos.md#orm).
...
Vamos a empezar por [crear un proyecto vacío](arranque.md#creacion-del-proyecto) en el que trataremos de **implementar una API para un «blog»**.
...
Por tanto [crearemos dos aplicaciones](aplicaciones.md#crear-una-aplicacion):
...
=== "Categorías"
    ...
    Una vez [creadas y aplicadas las migraciones](modelos.md#migraciones) del modelo, vamos a cargar algunos ^^datos de prueba^^. Para ello trabajaremos con [«fixtures»](modelos.md#fixtures).
...
- Usa [códigos HTTP](vistas.md#tipos-de-respuestas) correctos (200, 401, 403, 404, 405, 409, 422, 500).
...
- La ^^validación de datos^^ se realiza a través de [Pydantic :simple-pydantic:](https://docs.pydantic.dev/) utilizando [anotaciones de tipos](../../../fundamentos/modularidad/funciones.md#anotacion-de-tipos).
...
1. Necesitamos el [enrutador](#enrutadores).
2. Necesitamos el [esquema](#esquemas).
3.  - `#!python @router.get` :material-arrow-right-bold: Se trata de una petición `GET`.
    ...
    - `#!python list[PostSchema]` :material-arrow-right-bold: devolvemos una [lista](../../../fundamentos/estructuras/listas.md) de `PostSchema`.
4. El manejador recibe `request` por defecto pero ningún otro parámetro (en este caso).
5. Si añadimos un [docstring](../../../fundamentos/modularidad/funciones.md#documentacion) se verá reflejado en la documentación del punto de entrada.
...
Una vez hecho esto, podemos [levantar el servidor de desarrollo](arranque.md#primer-arranque) y visitar [http://localhost:8000/api/docs](http://localhost:8000/api/docs). Deberíamos ver una pantalla similar a la siguiente:

![Ninja API inicial](./images/api/ninjaapi-initial.png)

La «magia» de _Django Ninja_ hace que tengamos **documentación generada automáticamente** de nuestra API siguiendo la especificación **OpenAPI**[^1]. Nos aparecen todos nuestros [puntos de entrada](#puntos-de-entrada) y todos nuestros [esquemas](#esquemas).
...
Para ello debemos utilizar los llamados **«resolvers»**. Si queremos devolver un campo `field` debemos implementar el [método estático](../../../fundamentos/modularidad/poo.md#metodos-estaticos) `resolve_field()` en el esquema correspondiente.
...
=== "Modelo"
    ...
    Vamos a [añadir un método `save()`](modelos.md#guardar-de-forma-personalizada) al modelo `Post` para que se genere automáticamente el `slug` correspondiente al título del «post» al guardarlo en la base de datos:
...
=== "Esquema"
    ...
    Se hace necesario definir un **[esquema](#esquemas) de entrada** y un **[esquema](#esquemas) de salida** para el recurso «post». El esquema de entrada indicará los datos necesarios para crear un nuevo «post», mientras que el esquema de salida indicará los datos que se devolverán una vez creado el «post».
    ...
    1.  - No se incluyen los campos `id` y `slug` del esquema de entrada porque el `id` se genera automáticamente al crear el recurso y el `slug` se genera automáticamente a partir del `title` en el método `save()` del modelo.
        - Igualmente no se añade el campo `category` porque se verá en el próximo epígrafe [claves ajenas](#claves-ajenas).
...
Esta aproximación tiene la ventaja de que el valor de entrada de `vericode` es [validado](#validacion-de-datos) de forma automática por ~~Ninja~~ _Pydantic_. Si no cumple con la expresión regular indicada, se notificará un error en la respuesta HTTP correspondiente.
...
Una forma bastante sencilla de gestionar el error de recurso no encontrado es utilizar el método [`get_object_or_404()`](vistas.md#consulta-no-encontrada) de Django, que devuelve una respuesta con un código de estado [HTTP 404 (Not Found)](https://developer.mozilla.org/en-US/docs/Web/HTTP/Reference/Status/404) si el recurso no existe.
...
Para ello vamos a empezar [creando una aplicación](aplicaciones.md#crear-una-aplicacion) llamada `users` que gestione todo lo relacionado con los usuarios y la autenticación. Luego, añadimos el siguiente modelo en `users/models.py` para almacenar los tokens de autenticación:
...
2. Necesitamos vincularlo con la clase [`User`](autenticacion.md#usuario) que nos proporciona Django.
...
Una vez [creadas y aplicadas las migraciones](modelos.md#migraciones) del modelo, vamos a establecer los esquemas de entrada y salida, así como el manejador del punto de entrada para gestionar la creación de un nuevo «post» con portada:
