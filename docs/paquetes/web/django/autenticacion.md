...
| --- | --- | --- | --- | --- |
| [Claves ajenas](modelos.md#claves-ajenas) | `#!python from django.conf import settings` | `settings.AUTH_USER_MODEL` | Cadena de texto cualificada | `#!python 'auth.User'` |
| [Vistas](vistas.md) y/o [formularios](formularios.md) | `#!python from django.contrib.auth import get_user_model`  | `get_user_model()` | Función que devuelve el modelo | `django.contrib.auth.models.User` |
...
| Para uso en... | Acceso |
| --- | --- |
| [Vistas](vistas.md) | `request.user` |
| [Plantillas](plantillas.md) | `#!htmldjango {{ user }}` |
...
Se recomienda [crear una aplicación](aplicaciones.md#crear-una-aplicacion) `accounts` (o similar) donde implementar todos los artefactos necesarios para la autenticación de usuarios.

En las [URLs de primer nivel](urls.md#urls-de-primer-nivel) deberíamos incluir algo así:
...
1. El [«widget»](formularios.md#widgets) nos permite cambiar el elemento HTML al renderizar el campo de formulario.
...
??? example "Campos requeridos"
    ...
    Cuando implementamos un [formulario de modelo](formularios.md#formularios-de-modelo) los campos requeridos se infieren directamente desde el propio modelo. Pero hay ocasiones en las que no tenemos acceso directo al modelo, como es el caso que nos ocupa donde estamos utilizando el modelo `User` que proporciona Django.
...
!!! warning "Redirección para login"
    ...
    Si un usuario no logeado trata de acceder a una vista «protegida», será redirigido a la URL de «login» que venga especificada en la variable [`LOGIN_URL`](#url-de-login) del fichero `settings.py`.
