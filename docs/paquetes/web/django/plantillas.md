...
Las plantillas constituyen la **capa de presentación** del [modelo por capas](desarrollo-web.md#modelo-por-capas) visto en la introducción al desarrollo web.
...
Cuando hacemos referencia a una plantilla —habitualmente desde una [vista](vistas.md)— mediante una ruta (relativa), Django utiliza la siguiente estrategia para localizar la plantilla indicada:

<div class="annotate" markdown>
1. Busca dentro de la carpeta `/templates` de cada [aplicación](aplicaciones.md) del proyecto.(1)
...
:material-check-all:{ .blue } Los **espacios de nombres** son muy importantes a la hora de organizar las plantillas de nuestro proyecto Django. Ya lo dice el [Zen de Python](../../../entornos/desarrollo/python.md#zen-de-python): «Namespaces are one honking great idea -- let's do more of those!».
...
Desde la correspondiente [vista](vistas.md#vistas-con-parametros), tendremos que renderizar la plantilla anterior mediante el siguiente fragmento de código:
...
!!! note "Sin paréntesis"
    ...
    De aquí se deriva el hecho de que **no se pueden pasar parámetros** a funciones/métodos en plantillas. Para eso habría que hacer uso de [filtros](#filtros).
...
Para recorrer estructuras de datos [iterables](../../../fundamentos/modularidad/poo.md#iterables) en una plantilla se usa la etiqueta [`{% for %}`](https://docs.djangoproject.com/en/stable/ref/templates/builtins/#for) análoga al bucle [`for`](../../../fundamentos/flujo/bucles.md#la-sentencia-for) de Python.
...
!!! tip "`__str__()`"
    ...
    Recuerda que cuando usamos un objeto de modelo en una plantilla `#!htmldjango {{ post }}` se invoca automáticamente el método [`__str__`](../../../fundamentos/modularidad/poo.md#__str__) del modelo.

Desde la correspondiente [vista](vistas.md#pasando-un-contexto), tendremos que renderizar la plantilla anterior mediante el siguiente fragmento de código:
...
Los bucles en plantillas Django admiten también el [desempaquetado](../../../fundamentos/estructuras/tuplas.md#desempaquetado-de-tuplas) de secuencias tal y como se hacen en Python «tradicional».
...
Cuando usamos un [bucle](../../../fundamentos/flujo/bucles.md) en una plantilla Django tenemos acceso a ciertas [variables especiales](https://docs.djangoproject.com/en/stable/ref/templates/builtins/#for) que nos pueden facilitar la lógica a implementar.
...
Django proporciona la etiqueta [`{% if %}`](https://docs.djangoproject.com/en/stable/ref/templates/builtins/#if) para llevar a cabo comprobaciones en el código de una plantilla. Funciona de manera análoga a la sentencia [`if`](../../../fundamentos/flujo/condicionales.md#la-sentencia-if) de Python.
...
Para construir condiciones más complejas podemos hacer uso de los mismos [operadores lógicos](../../../fundamentos/flujo/condicionales.md#operadores-logicos) que en Python «tradicional»: `#!python and`, `#!python or` y `#!python not`.
...
Ya hemos visto que las [URLs definidas](urls.md#urls) en `urls.py` ~~pueden~~ deben disponer de un nombre que las identifique.
...
1.  - `#!html <title>Technology | Blog</title>` si se envía _título_ en el [contexto](vistas.md#pasando-un-contexto) `#!python {'title': 'Technology'}` de la vista.
    - `#!html <title>Blog</title>` si no se envía _título_ en el [contexto](vistas.md#pasando-un-contexto) `#!python {}` de la vista.

2. Se define un bloque para el contenido **propio** de la página.

!!! tip "Ubicación de la plantilla base"

    La plantilla base (raíz del proyecto) se entiende que será ^^compartida^^ por todas las aplicaciones del proyecto. Por ello, puede ser interesante crear una aplicación [`shared`](aplicaciones.md#aplicacion-compartida) que contenga la mencionada plantilla `base.html`.
...
1. Es perfectamente válido utilizar [variables](#variables) o [etiquetas](#etiquetas) dentro de las plantillas a incluir.
...
A diferencia de las [etiquetas](#etiquetas-personalizadas) los filtros ^^deben recibir^^ **un argumento** (y eventualmente otro). El primer argumento es el valor de la variable a la que aplicamos el filtro y el segundo argumento es opcional y permite modificar el comportamiento predefinido.
