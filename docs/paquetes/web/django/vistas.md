...
En el [MVC](desarrollo-web.md#framework-web) de Django se podría decir que las **vistas** se corresponden con los **controladores**. En esta sección veremos cómo manejar las vistas y explotar sus principales funcionalidades.
...
2. La función `render()` siempre recibe como primer parámetro la petición `HttpRequest` y como segundo parámetro la [ruta a la plantilla](plantillas.md#ubicacion).
...
2. Hacemos una consulta y [recuperamos todos los objetos](modelos.md#todos-los-objetos) de tipo «post» existentes en la base de datos.
...
Hay ocasiones, especialmente en el manejo de [formularios](formularios.md) que nos interesa detectar el método de la petición HTTP para tomar una acción u otra en función de ello.
...
Los [decoradores](../../../fundamentos/modularidad/funciones.md#decoradores) en Python permite modificar el comportamiento de una función de un modo poco intrusivo y bastante elegante.
...
- [Conversores personalizados en URLs](urls.md#conversores-personalizados).
...
Django proporciona un «shortcut» (atajo) para cuando queremos [recuperar un objeto](modelos.md#un-unico-objeto) pero —en vez de lanzar una excepción de tipo `DoesNotExist`— devolver un [`Http404`](https://docs.djangoproject.com/en/stable/topics/http/views/#django.http.Http404).
